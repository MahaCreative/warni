...
const Th = ({ className, children }) => {
    return (
        <th
            ...
            className={clsx(
                className,
                "whitespace-nowrap px-6 py-3 text-left text-sm font-semibold text-white"
            )}
        >
            {children}
        </th>
        ...
        <Menu.Item>
            {({ active }) => (
                <Link
                    ...
                    className={clsx(
                        className,
                        active ? "bg-gray-50 text-black" : " text-white",
                        "block w-full py-2 px-4 text-left text-sm font-medium text-white hover:bg-gray-800/20 backdrop-blur-sm hover:text-white-600"
                    )}
                >
                    {children}
                </Link>
    ...
    return (
        <div className="relative">
            <Menu>
                ...
                <Menu.Items className="absolute top-0 right-7 z-10 w-56 divide-y overflow-hidden rounded-md border bg-gray-800/20 backdrop-blur-sm py-1 text-left shadow-sm">
                    {children}
                </Menu.Items>
            </Menu>
        </div>
